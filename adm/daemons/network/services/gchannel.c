...
{      return;
	...
	CHANNEL_D->do_sys_channel("sys", "prepare to send gchannel message.");
#endif
...
void incoming_request(mapping info)
{
	...
	if (info["NAME"]) {
		...
		if(undefinedp(info["CNAME"])) {
			...
			set("name", sprintf("%s@%s", info["USRNAME"], info["NAME"]));
		} else {
			set("channel_id", sprintf("%s(%s@%s)", info["CNAME"], info["USRNAME"], info["NAME"]));
			set("name", sprintf("%s@%s", info["CNAME"], info["NAME"]));			
		}
		CHANNEL_D->do_channel(this_object(), lower_case(info["CHANNEL"]) + (info["EMOTE"] ? "*" : ""), info["MSG"]);
