...
int startup_udp()
{
	...
	CHANNEL_D->do_sys_channel("sys", sprintf("UDP port #%d\n",my_port));
	return 1;
}
...
void send_udp(string host, int port, string msg)
{
	...
	if( !ACCESS_CHECK(previous_object()) && file_name(previous_object())[0..strlen(AUX_PATH) - 1] != AUX_PATH ) {
		return;
	}
...
//	if(previous_object()) return;
	...
	if( args["NAME"] && undefinedp(muds[args["NAME"]]) && member_array(name, muds_name) == -1 ) {
		int count;
		if( !undefinedp(args["USERS"]) && sscanf(args["USERS"], "%d", count) == 1 ) {
			if( count < 0 ) {
				return;
			}
		}
		muds_ip += ({ args["HOSTADDRESS"] });
		muds_port += ({ args["PORTUDP"] });
		muds_name += ({ htonn(args["NAME"]) });
	}
...
void send_shutdown()
{
	...
	i = sizeof(mud_names);
	while( i-- ) {
		SHUTDOWN->send_shutdown(muds[mud_names[i]]["HOSTADDRESS"], muds[mud_names[i]]["PORTUDP"]);
	}
	socket_close(socket_id);
...
void init_database()
{
	...
	// send a startup and request a mudlist, but not to ourselves.
	if((bootsrv[0] != this_host["HOSTADDRESS"]) || (bootsrv[1] != mud_port())) {
		message = sprintf("@@@%s%s@@@\n", DNS_STARTUP, start_message());
		send_udp(bootsrv[0], SRVC_PORT_UDP(bootsrv[1]), message);
		MUDLIST_Q->send_mudlist_q(bootsrv[0], SRVC_PORT_UDP(bootsrv[1]));
	}
