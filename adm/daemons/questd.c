...
int holiday_bonus=200;   	// 05/20/2008 fixed to x2 reward  注：额外的任务奖励。 
int chance = 20;
int level_chance = 80;
...
//	I don't like 寻 at higher level (>=90), 强制性转换
   if(who->query("env/onlymaze")==1){
   	   //	CHANNEL_D->do_sys_channel("sys", sprintf("%s\n","quest") );	
	if (F_LEVEL->get_level(combatexp)>= level_chance && quest["quest_type"] == "寻" 
	&& (random(100)<100 || who->query("chuili_unfinished"))) { 
		tag = "_repeat";
		quest = ([
			"quest":	"探访垂笠老僧",
