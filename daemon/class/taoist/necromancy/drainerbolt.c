...
int cast(object me, object target)
{
	...
	if( lvl < 40 ) {
		return notify_fail("『紫幽之箭』需要40级的三清道术！\n");
	}
	...
	if( me->query("mana") < cost || me->query("sen") < cost ) {
		return notify_fail("『紫幽之箭』至少需要"+ cost + "点心神和法力。\n");
	}
	if( userp(me) ) {
		me->add("mana", -cost);
	}
	msg = BBLU + HIM "$N口中喃喃地念著咒文，左手一挥，手中聚起一团紫光射向$n！\n" NOR;
	me->set_temp("last_sqpfm_used", 2);
	me->add_temp("apply/spells", lvl*2);
	if( COMBAT_D->do_magic_attack( me, target, "spells", "gin", 0) ) {
		msg += BBLU + HIR "结果「嗤」地一声，紫光从$p"+HIR"身上透体而过，拖出一条长长的七彩光气，\n"
